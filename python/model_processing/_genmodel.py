...
class GenModel:
    ...
    def __init__(self, model_params, model_marginals):
        ...
        self.events = list()
        self.edges = {}
        self.marginals = read_model_marginals(filename=model_marginals)
        self.read_model_params(filename=model_params)

    def read_model_params(self, filename):
        ...
        with open(filename, "r") as file:
            ...
            if strip_line == "@Event_list":
                ...
                while strip_line[0] == '#':
                    ...
                    event = RecEvent(event_type, seq_type, seq_side, priority, nickname)
                    ...
                    while strip_line[0] == '%':
                        ...
                        realization = EventRealization(name, value, index)
            ...
            if strip_line == "@Edges":
                ...
                while strip_line[0] == '%':
                    ...
                    if not parent in self.edges:
                        self.edges[parent] = AdjacencyList()

                    if not child in self.edges:
                        self.edges[child] = AdjacencyList()
...
class RecEvent:
    ...
    """
    def __init__(self, event_type, seq_type, seq_side, priority, nickname=None):
    ...
    def get_realization_vector(self):
        ...
        if self.event_type == 'GeneChoice':
            ...
            tmp = numpy.empty(len(self.realizations), dtype=type(self.realizations[0].value))
    ...
    for event in model.events:
        if event.name == event_name:
            ...
            while not explored:
                ...
                for parent in parents_names:
                    ...
                    for parent1 in parent_parents:
                        ...
                        if event_cluster.count(model.get_event(parent1)) == 0:
                            event_cluster.append(model.get_event(parent1))
            ...
            for event1 in event_cluster:
                dimension_list.append(model.marginals[0][event1.nickname].shape[-1])
            ...
            if event.event_type != "DinucMarkov":
                ...
                for event2 in event_cluster:
                    if event2.name not in model.edges:
                        event_parents = []
                    else:
                        event_parents = model.edges[event2.name].parents
                    # print(event_parents)
                    dimension_list = []
                    for event3 in event_cluster:
                        if (event_parents.count(event3.name) > 0) or (event2.name == event3.name):
                            dimension_list.append(model.marginals[0][event3.nickname].shape[-1])  # same as getting the number of realizations
                        else:
                    ...
                    swapped_marginal = copy.deepcopy(model.marginals[0][event2.nickname])
                    swapped_dimension_names = copy.deepcopy(model.marginals[1][event2.nickname])
                ...
                sum_axes = list(range(0, len(reshaped_marginals[0].shape)))
                sum_axes.remove(find(numpy.asarray(dimension_names_list) == event.nickname))
                if averaging_list is not None:
                    ...
                    for dimension_name in dimension_names_list:
                        if dimension_name != event.nickname and all(numpy.asarray(averaging_list) != dimension_name):
                            sum_axes.remove(find(numpy.asarray(dimension_names_list) == dimension_name))
...
def read_model_marginals(filename, dim_names=False):

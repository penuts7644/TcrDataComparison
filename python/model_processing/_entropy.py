        ...
        if event.name == event_name:
            ...
            while not explored:
                ...
                for parent in parents_names:
                    ...
                    for parent1 in parent_parents:
                        next_parents.append(parent1)
                        if event_cluster.count(model1.get_event(parent1)) == 0:
                            event_cluster.append(model1.get_event(parent1))
            ...
            for event1 in event_cluster:
                dimension_list.append(model1.marginals[0][event1.nickname].shape[-1])
                dimension_names_list.append(event1.nickname)
            ...
            if event.event_type != "DinucMarkov":
                ...
                for event2 in event_cluster:
                    if event2.name not in model1.edges:
                        event_parents = []
                    else:
                        event_parents = model1.edges[event2.name].parents
                    # print(event_parents)
                    dimension_list = []
                    for event3 in event_cluster:
                        if ((event_parents.count(event3.name) > 0) or (event2.name == event3.name)):
                            dimension_list.append(model1.marginals[0][event3.nickname].shape[-1])  # same as getting the number of realizations
                        else:
                    ...
                    swapped_marginal = copy.deepcopy(model1.marginals[0][event2.nickname])
                    swapped_dimension_names = copy.deepcopy(model1.marginals[1][event2.nickname])
                    ...
                    while not is_sorted:
                        for i in range(0, len(swapped_marginal.shape)):
                            if i == len(swapped_marginal.shape) - 1:
                                is_sorted = True
                                break
                            elif (find(numpy.asarray(dimension_names_list) == swapped_dimension_names[i]) > find(numpy.asarray(dimension_names_list) == swapped_dimension_names[i + 1])):
                ...
                if event_name not in model1.edges:
                    ...
                    event_parents = model1.edges[event_name].parents
                dimension_list = []
                for event4 in event_cluster:
                    if ((event_parents.count(event4.name) > 0) or (event_name == event4.name)):
                        dimension_list.append(model1.marginals[0][event4.nickname].shape[-1])  # same as getting the number of realizations
                    else:
                        dimension_list.append(1)
                ...
                final_array *= numpy.log2(reshaped_event_marginals + pseudo_count)
                if debug_output:
                    return -final_array
            ...
            if event.event_type == "DinucMarkov":
                ...
                for junction in junction_list:
                    ...
                    for event5 in model1.events:
                        if event5.event_type == "Insertion" and event5.seq_type == junction:
                            insertion_event = event5
                            junction_found = True
                            break
...
def compute_markov_steady_entropy(dinuc_event, model1, model2, event_cluster, dimension_list, dimension_names_list, insertion_event):
    ...
    for event in event_cluster:
        if event.name not in model1.edges:
            event_parents = []
        else:
            event_parents = model1.edges[event.name].parents
        # print(event_parents)
        dimension_list = []
        tot_dimension = 1
        for event1 in event_cluster:
            if event_parents.count(event1.name) > 0:  # Dimension for the dinuc is 1 since we'll store vectors and matrices as dtype
                dimension_list.append(model1.marginals[0][event1.nickname].shape[-1])  # same as getting the number of realizations
                tot_dimension *= model1.marginals[0][event1.nickname].shape[-1]
            else:
                dimension_list.append(1)
        # print("Tot dimension:" + str(tot_dimension))

        # Swap marginals in the correct order for model1 contributions
        swapped_marginal = copy.deepcopy(model1.marginals[0][event.nickname])
        swapped_dimension_names = copy.deepcopy(model1.marginals[1][event.nickname])
    ...
    for i in range(0, tot_dimension):
        ...
        for [transition_matrix, mono_nt_vect, conditional_nt_mat] in [(transition_matrix_1, mono_nt_vect_1, conditional_nt_mat_1), (transition_matrix_2, mono_nt_vect_2, conditional_nt_mat_2)]:
            ...
            [eigenValue, eigenVect] = numpy.linalg.eig(transition_matrix.transpose())
            ...
            stat_dist = numpy.divide(stat_dist, sum(stat_dist))  # although should already be a unit vector

            # print("Stationnary distribution:" + str(stat_dist))

            mono_nt_vect[i] = stat_dist
            conditional_nt_mat[i] = numpy.diag(numpy.asarray(stat_dist).reshape(4))
        # print(numpy.asarray(stat_dist).reshape(4))
        # print(numpy.diag(numpy.asarray(stat_dist).reshape((4))))

        mono_nt_cross_entropy[i] = numpy.nansum(numpy.multiply(mono_nt_vect_1[i], numpy.log2(mono_nt_vect_2[i])))  # SHOULD PROBABLY ADD PSEUDOCOUNTS
        cross_entropy_production_rate[i] = numpy.nansum(numpy.nansum(numpy.multiply(conditional_nt_mat_1[i] * transition_matrix_1, numpy.log2(transition_matrix_2))))
    ...
    if dinuc_event.name in model1.edges and model1.edges.parents.count(insertion_event.name):
...
def compute_models_dkl(model1, model2, type_list=None):
    """Compute the Kullback Leibler divergence two recombination models.
    Returns D_KL(model1||model2). If a list of event types are given, use only
    those events to compute the divergence.

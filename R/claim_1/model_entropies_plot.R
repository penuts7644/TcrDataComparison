...
process_entropies <- function(data, entropies) {
  ...
  tmp2$value <- apply(tmp2, 1, function(x) {
    return (((as.numeric(x['entropy.x']) + as.numeric(x['entropy.y']) + as.numeric(x['entropy'])) / 3) / 20)
  })
  for (i in seq(1, length(subject_list), 2)) {
...
# PLOT VARIABLES
# ----------
plot_y_left <- 'KL divergence (bits)'
plot_y_right <- 'Average KL divergence (bits)'
...
entr_compare <-
  ...
  scale_y_continuous(
    sec.axis = sec_axis(~ .*20, name = plot_y_right)
  ) +
  theme_bw() +
  theme(
